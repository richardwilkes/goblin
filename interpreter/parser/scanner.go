...
func (s *Scanner) Scan() (tok int, lit string, pos interpreter.Position, err error) {
retry:
	s.skipBlank()
	pos = s.pos()
	switch ch := s.peek(); {
...
func (s *Scanner) pos() interpreter.Position {
	return interpreter.Position{Line: s.line + 1, Column: s.offset - s.lineHead + 1}
}
